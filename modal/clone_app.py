# Modal 음성 복제 서버 (c.ai 방식 = zero-shot) — 학습 없이 샘플 몇 초로 그 목소리를 흉내낸다.
#
# RVC(rvc_app.py)와의 차이:
#   RVC   : 목소리마다 미리 학습된 .pth 모델 필요 (커뮤니티에서 받아옴) → 음색 재현 정교, 학습 데이터 언어에 억양이 끌림
#   Clone : 참조 음성 5~20초만 있으면 즉시 사용 (학습 없음) → 다국어 네이티브(한국어 OK), 각자 자기 목소리 업로드 가능
#
# 배포:  modal deploy modal/clone_app.py
# 엔드포인트:
#   GET  /warm                     — 워밍업 + 등록된 목소리 목록
#   POST /add_sample {name,url}    — 참조 음성(wav/mp3 URL) 등록 (볼륨에 영구 저장)
#   POST /speak?voice=X&lang=ko    — body=대사 텍스트 → 그 목소리의 raw PCM(24k mono s16le)
import glob
import io
import os
...
@app.cls(
    ...
    secrets=[modal.Secret.from_name("rvc-add-token")],
)
class CloneServer:
    @modal.enter()
    def setup(self):
